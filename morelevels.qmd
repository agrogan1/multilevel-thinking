...
$$w_{0k} + u_{0j} + v_{0i} + e_{itjk}$$ {#eq-fourlevel}
...
::: {.callout-caution collapse="false"}
Similar to my caution above, note that, to keep the example somewhat realistic with regard to the official languages of the United Nations, I am adding 6 languages to the data. Remember, however, that 6 groupings may be too few categories to use as an additional random effect (@sec-indicator-variables-random-effects), and that it is worth exploring whether it might be more appropriate to model these languages as indicator variables, rather than as random effects (@sec-indicator-variables-random-effects).
:::

### Equation

The terms in the equation below are similar to those in @eq-fourlevel. Note, however, that here, for purposes of exposition, I am working with the cross sectional data. `country`, and the new variable, `language` are *cross-classified* levels in the data in that neither is hierarchically nested inside the others: residents of the same country will not all speak the same language, and speakers of the same language will not all live in the same country. I would use a random effect $l$ for language, except for the fact that it would create confusion because of the similarity of $l$ and the number $1$. Therefore, I use the next available letter of the alphabet for language, $m$.
...
Again, terms are similar to the terms used in @eq-MLM-longitudinal and @eq-fourlevel, with the exception that in @eq-crossclassified, we are talking about family *i* in country *j*, speaking language *m*.

# The Longitudinal Multilevel Model {#sec-longitudinal}

> "Mathematics is the art of giving the same name to different things." [@Poincare1908] \index{Henri Poincaré}
...
In longitudinal data, we consider the *first level* to be that of *time points*, or *study waves*, which we sometimes call the *person-observation*.[^personobservation] \index{person-observation} The *second level* is then the individual or family. 
...
### Unconditional Model {#sec-unconditional-longitudinal}
...
$$\text{ICC} = \frac{\text{var}(u_{0j})}{\text{var}(u_{0j}) + \text{var}(v_{0i}) + \text{var}(e_{ij})}$$ {#eq-ICCunconditional3A}
...
$$\text{ICC} = \frac{\text{var}(u_{0j}) + \text{var}(v_{0i})}{\text{var}(u_{0j}) + \text{var}(v_{0i}) + \text{var}(e_{ij})}$$ {#eq-ICCunconditional3B}

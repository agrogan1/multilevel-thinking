...
Counter-intuitively, and surprisingly, the mathematics of estimating models with cross-sectional clustered data easily generalizes to longitudinal data. In cross sectional clustered data, we imagine *individuals or families clustered in neighborhoods, schools, or countries*.
...
In longitudinal data, we consider the *first level* to be that of *time points*, or *study waves*, which we sometimes call the *person-observation*.[^personobservation] The *second level* is then the individual or family. 
...
In longitudinal multilevel models, the variable for *time* assumes a special role as we are often visualizing a *growth trajectory* over the course of time.

Imagine a model as follows where *group* is a (1/0) variable for membership in one of two groups:
...
|-------|--------------------------------|----------------------------|
| 0     | $\beta_0$                      | $\beta_t$                            |
| 1     | $\beta_0 + \beta_\text{group}$ | $\beta_t + \beta_\text{interaction}$ |
...
ggplot(data = NULL) + 
  ...
  annotate("text", 
           x = 1, 
           y = .6, 
           label = "intercept: beta[0] + beta[group]",
           parse = TRUE,
           color = "#31688EFF") +
  annotate("text", 
           x = 5, 
...
mixed outcome t warmth physical_punishment group HDI || country: warmth || id: t
...
Examining the regression results, the results of the model suggest that child outcomes improve over time. Better child outcomes are again associated with parental `warmth`, and parental use of `physical_punishment` is associated with reduced child outcomes. `HDI` is again not associated with outcomes.
...
The Stata command that we use to analyze this data is below. Because this ends up being a somewhat longer line of syntax, we use `///` to ensure that Stata continues to the next line:

```{stata, eval = FALSE, echo=TRUE}

mixed outcome c.t##(c.warmth c.physical_punishment i.group c.HDI) || ///
country: warmth || id: t

```

In Stata, when estimating interactions, it is especially important to denote which variables are *continuous*, and which variables are *categorical*. We do this by prefixing variable names with `c.` and `i.` respectively.

```{r, child=c('./simulate-and-analyze-multilevel-data/table2B.md')}
```

Examining the regression results, the results of the model again suggest that child outcomes improve over time. Better child outcomes are again associated with parental `warmth`, and parental use of `physical_punishment` is associated with reduced child outcomes. `HDI` is again not associated with outcomes.
...
* Multilevel Model: `mixed outcome t warmth physical_punishment group HDI || id:`
* Fixed Effects Model: `xtreg outcome t warmth physical_punishment group HDI, i(id) fe`
...
Second, estimates for any quantities that do not vary over time, in this case, `group` membership, and `HDI`, are not available from the fixed effects regression. 
...
mixed outcome t warmth mean_warmth physical_punishment mean_physicalpunishment group HDI || id:
...
Note a couple of things from this table. First, results from the correlated random effects model, and the fixed effects regression model are exactly the same for *time varying* variables, `t`, `warmth`, and `physical_punishment`. Again, these coefficients for *time varying* variables are estimated with statistical control for all time invariant characteristics of study subjects, whether those characteristics are observed, or unobserved. Secondly, unlike the fixed effects regression, coefficients for *time invariant* variables, e.g. `group`, `HDI`, mean levels of `warmth`, and mean levels of `physical_punishment` are provided, while they are not provided in the fixed effects model. 

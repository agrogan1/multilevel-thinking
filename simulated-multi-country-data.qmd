...
`identity` is a hypothetical designation of an identity--such as race, ethnicity, or gender identity. In this simulated data, identity has two categories--for ease of presentation--but could easily be a more than two category variable. `identity` is also a *Level 1* variable.

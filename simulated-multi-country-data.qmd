...
`identity` is a hypothetical designation of an identity--such as race, ethnicity, or gender identity. In this simulated data, identity has three categories--for ease of presentation--but could easily be a two category variable, or a variable with more than three categories. `identity` is also a *Level 1* variable.

...
@eq-contextual is be a model that includes parenting as a predictor of the outcome at two different levels. $\text{parenting}_{ij}$ is a parenting behavior--whether discipline or warmth--at an *individual* level, while $\overline{\text{parenting}_{.j}}$ is a *contextual variable* representing the average level of parenting--whether discipline or warmth--at the *country* level. @eq-contextual is thus testing whether parenting at the country level--a so-called country-level effect--has an association with the outcome over and above individual level parenting behavior.

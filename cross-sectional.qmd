# The Cross Sectional Multilevel Model {#sec-crosssectional}

> "Mathematical Science shows us what is. It is the language of unseen relations between things. But to use & apply that language we must be able fully to appreciate, to feel, to seize, the unseen, the unconscious. Imagination too shows us what is, the is that is beyond the senses.” [@Lovelace1992] \index{Ada Lovelace}

> "I’m often asked if there is something I think writers ought to do, and recently in an interview I heard myself say: 'Several things. Love words, agonize over sentences. And pay attention to the world.'" [@Sontag2007] \index{Susan Sontag}
...
p0 + 
  ...
  geom_smooth(aes(color = country), # smoothers with country color
              method = "lm") + 
  labs(title = "Desirable Outcome as a Function of \nIntervention or Treatment",
...
Here $y_{ij}$ is the dependent variable, or outcome for the model. We note that the $ij$ subscripts indicate that this is outcome $y$ for individual $i$ in country $j$. Note that the outcome is at Level 1, or the level of individuals. $\beta_{0j}$ is a regression intercept, and the other $\beta$'s[^beta:] are regression slope parameters. $x_{ij}$ and $z_{ij}$ are independent variables. I note that in this discussion I am *not* considering a model in which there are repeated observations on the same individuals, although the multilevel model is certainly extensible to such cases (@sec-longitudinal). $u_{0j}$ is a *random intercept* for the $\beta_{0j}$ term, and $u_{1j}$ is a *random slope* for the $\beta_{1j}$ term, indicating that we are modeling cross country variation in these parameters. \index{random intercept} \index{random slope} The other $\beta$ terms are not modeled as having random country level variation, although this could certainly be a possibility in subsequent models. 
...
$$y_{ij} = \beta_0 + \beta_1 x_{ij} + \beta_2 z_{ij} + \beta_3 w_{j} + u_{0j} + u_{1j} \times x_{ij} + e_{ij}$$ {#eq-MLM}
...
In this formulation of the equation, the nature of the random effects is more clear, and merits discussion. Notice that we have included a *random intercept*, $u_{0j}$ as well as a *random slope*, $u_{1j} \times x$. The *random intercept*, $u_{0j}$, indicates that there is variation in the *intercept* of the country specific regression lines, as is true in @fig-data. The *random slope* term associated with $x_{ij}$, $u_{1j} \times x_{ij}$, indicates that we are allowing for the possibility of variation in the *slope* of the regression lines that is associated with $x_{ij}$, in this case, the slope of parental warmth, as is possibly suggested in @fig-data. 
...
$$\text{ICC} = \frac{\text{var}(u_{0j})}{\text{var}(u_{0j}) + \text{var}(e_{ij})}$$ {#eq-ICC}
...
## Indicator Variables, Random Intercepts and Random Slopes, and Identities {#sec-indicator-variables-random-effects}
...
When teaching and learning about statistical software, I find it easiest to talk about *quantitative* variables as *continuous* or *numeric* variables, and *qualitative* variables as *categorical* variables.[^finer_distinctions]

[^finer_distinctions]: I recognize that there are much finer distinctions to be made here, many of which @Freedman1991--as well as other introductory texts--go on to make. However, for the purposes of explication in this book, I find the *continuous/numeric* versus *categorical* variable distinction to be the most useful and the one that most readily facilitates communication with faculty colleagues and with students. 
...
# ::: {.callout-warning}
# #### Contextual Variables As Indicator Variables
# 
# If a variable is actually a variable that is aggregated to Level 2, sometimes called a contextual variable (@sec-levels), it may be difficult to estimate multiple categorical contextual variables at the same time. \index{contextual variables}
# 
...
$$\text{outcome}_{ij} = \beta_0 + \beta_1 \text{parental warmth}_{ij} + \beta_2 \text{physical punishment}_{ij} +$$ {#eq-MLMsubstantive2}

$$\beta_3 \text{identity}_{ij} + \beta_4 \text{intervention}_{ij} + \beta_5 \text{HDI}_{ij} + $$

$$u_{0j} + u_{1j} \times \text{parental warmth} + e_{ij}$$ 

### Measured and Unmeasured Variation {#sec-measured-and-unmeasured-variation}
...
![Measured and Unmeasured Variation](measured-and-unmeasured-variation.png){width=75%}

$$\!$$ {#eq-measured-and-unmeasured-variation}
...
| $cov(u_{0j}, u_{1j})$ | What is the covariance of the country specific intercept and country specific slope. Is the country specific intercept related to the country specific slope? |   
